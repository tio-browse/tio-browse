...
int main(int argc, char *argv[]) {
  ...
  tioError = TIO_Create_Mesh(
      fileID, stateID, "mesh", &meshID, TIO_MESH_QUAD_COLINEAR,
  ...
  int sum_x_y_z[] = {0, 1, 1, 2, 2, 3, 1, 2, 2, 3, 3, 4,
                     2, 3, 3, 4, 4, 5, 3, 4, 4, 5, 5, 6};
  tioError = TIO_Write_QuadQuant_Chunk(fileID, quantID, 0, TIO_XFER_COLLECTIVE,
                                       TIO_INT, sum_x_y_z, nullptr);
  if (tioError != TIO_SUCCESS) {
    PrintTyphonIOErrorAndExit(tioError, "Error in TIO_Write_QuadQuant_Chunk");
  }
