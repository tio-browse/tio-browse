...
static PyObject *Redirector_new(PyTypeObject *type, PyObject *args,
                                PyObject *kwds) {
...
static int Redirector_init(RedirectorObject *self, PyObject *args,
                           PyObject *kwds) {
  return 0;
}

static PyTypeObject RedirectorObject_Type = {
    PyVarObject_HEAD_INIT(NULL, 0).tp_name = "redirector.redirector",
...
static struct PyModuleDef redirector_moduledef = {
    PyModuleDef_HEAD_INIT, "redirector",
...
PyMODINIT_FUNC PyInit_redirector(void) {
  ...
  if (PyModule_AddObject(m, "redirector", (PyObject *)&RedirectorObject_Type) <
      0) {
...
static PyObject *py_quit(PyObject *, PyObject *) {
  resultString =
      "Use reset() to restart the interpreter; otherwise exit your "
      "application\n";
  return Py_None;
}
...
static PyMethodDef console_methods[] = {
    ...
    {"reset", py_reset, METH_VARARGS,
     "reset the interpreter and clear the console"},
    ...
    {"version", py_version, METH_VARARGS,
     "print information about versions of python, system and compilers"},
    {NULL, NULL, 0, NULL}};

static struct PyModuleDef console_moduledef = {
    PyModuleDef_HEAD_INIT, "console",
...
QPyConsole *QPyConsole::getInstance(QWidget *parent,
...
bool QPyConsole::py_check_for_unexpected_eof() {
  ...
  if (errobj != NULL && (pystring = PyObject_Str(errobj)) != NULL &&
      /* str(object) */  // what is difference between str and repr in python?
...
// retrieve back results using the python internal stdout/err redirectory (see
// above)
QString QPyConsole::interpretCommand(const QString &command, int *res) {
  ...
  *res = 0;
  if (!command.startsWith('#') &&
      (!command.isEmpty() || (command.isEmpty() && lines != 0))) {
    if (command.contains("license()", Qt::CaseInsensitive)) {
      QString result =
          "WARNING: Can't type LICENSE() as Interactive Prompt is Disabled. "
    ...
    if (command.contains("help()", Qt::CaseInsensitive)) {
      QString result =
          ...
          "version() - print information about versions of python, system and "
          "compilers\n";
      return result;
    }
    this->command.append(command);
    py_result = Py_CompileString(this->command.toLocal8Bit().data(), "<stdin>",
                                 Py_single_input);
    if (py_result == 0) {
      multiline = py_check_for_unexpected_eof();
      if (!multiline) {
        if (command.endsWith(':')) multiline = true;
      }
    ...
    if ((lines != 0 && command == "") || (this->command != "" && lines == 0)) {
      ...
      QString result = resultString;
      resultString = "";
      if (command != "") QConsole::interpretCommand(command, res);
      return result;
...
QStringList QPyConsole::suggestCommand(const QString &cmd, QString &prefix) {
  ...
  if (!cmd.isEmpty()) {
    do {
      ...
      resultString =
          resultString.trimmed();  // strip trialing newline //resultString does
                                   // not work until redirector/catcher does.
...
void QPyConsole::dataIntoConsole(QString name, const int ND, QList<int> DIMS,
  ...
  PyObject *pArray =
      PyArray_SimpleNewFromData(ND, (npy_intp *)dims, TypeInt, data);
